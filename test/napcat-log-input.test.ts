...
describe('dsh-qq-bridge — NapCat self log input', () => {
  const opts = { selfQq: 10001, commandPrefix: '/dsh' }

  it('parses self private send lines', () => {
    const line = '08-16 11:07:50 [info] Tomoyo | 发送 -> 私聊 (10001) /dsh hello'
  ...
  it('ignores non-command and non-self lines', () => {
    expect(parseNapcatSelfLogLine('08-16 11:00:00 [info] Tomoyo | 发送 -> 私聊 (10001) hello', opts)).toBeNull()
    expect(parseNapcatSelfLogLine('08-16 11:00:00 [info] Tomoyo | 发送 -> 私聊 (20002) /dsh hello', opts)).toBeNull()
    expect(parseNapcatSelfLogLine('08-16 11:00:00 [info] Tomoyo | 接收 <- 私聊 (10001) /dsh hello', opts)).toBeNull()
  })

  it('strips ansi color sequences before parsing', () => {
    const line = '08-16 11:07:50 [\u001b[32minfo\u001b[39m] Tomoyo | 发送 -> 私聊 (10001) /dsh hello'
