...
export async function startDshWebBackground(options) {
    ...
    if (!cwdStat?.isDirectory())
        throw new Error(`DSH directory does not exist: ${options.cwd}`);
...
export async function getDshWebStatus(port = 3080) {
    const url = `http://127.0.0.1:${port}`;
...
export async function stopDshWeb() {
    const pid = await readPidFile();
    if (pid === null)
        return { pid, stopped: false, message: `未找到 pid 文件: ${DSH_WEB_PID_PATH}` };
...
function spawnDshWeb(cwd, command, logFd) {
...
async function readLivePidFile() {
    const pid = await readPidFile();
