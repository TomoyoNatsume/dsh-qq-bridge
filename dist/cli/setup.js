...
export async function runSetup() {
    ...
    try {
        ...
        const platform = await prompt.choice('选择 QQ 接入方式', ['NapCat / OneBot', '腾讯官方 QQ Bot'], 'NapCat / OneBot');
...
async function preflightBase(prompt) {
    if (platform() !== 'linux') {
...
async function runNapcatSetup(prompt) {
    if (!preflightNapcat())
        return;
    ...
    printRestartDshWebGuidance(() => printVerifyGuidance(answers, '重启后请'), `如果发送 ${formatCommandExample(answers.commandPrefix, 'ping')} 后没有响应，请查看 NapCat 日志确认 QQ 是否登录成功。`);
}
...
async function runOfficialSetup(prompt) {
    console.log('\n请先到这里配置你的 QQ 机器人:');
    ...
    await configureDshSettings(answers.permissionDefault);
    printRestartDshWebGuidance(() => printOfficialVerifyGuidance(answers, '重启后请'), `如果发送 ${formatCommandExample(answers.commandPrefix, 'ping')} 后没有响应，请查看 DSH web 日志和 QQ 开放平台机器人状态。`);
}
...
async function collectOfficialAnswers(prompt) {
    const common = await collectCommonAnswers(prompt);
...
async function promptPermissionDefault(prompt) {
    ...
    return await prompt.choice('设置 permission.defaultPreset', ['workspace-write', 'danger-full-access', '保持现有 settings.yaml'], 'workspace-write');
}
...
async function configureDshSettings(permissionDefault) {
...
function printRestartDshWebGuidance(printVerify, noResponseHint) {
