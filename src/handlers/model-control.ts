...
import { DIR_COMMAND, DirectorySwitcher, resolveUserPath } from './directory.js'
...
const FALLBACK_REASONING_EFFORTS = ['off', 'low', 'medium', 'high', 'max'] as const
...
export class BridgeControlHandler implements Handler {
  name = 'bridge-control'

  constructor(
    private readonly controller: ModelSelectionController,
    private readonly directorySwitcher?: DirectorySwitcher,
  ...
  private async runPermission(ctx: HandlerContext, sessionKey: string): Promise<void> {
  ...
  private async runDir(ctx: HandlerContext, sessionKey: string): Promise<void> {
  ...
  private async runModel(ctx: HandlerContext, sessionKey: string): Promise<void> {
  ...
  private async runReasoningEffort(ctx: HandlerContext, sessionKey: string): Promise<void> {
    ...
    if (!effort) {
      ...
      const info = (await this.controller.listModels(sessionKey)).find(entry =>
    ...
    try {
      ...
      const info = (await this.controller.listModels(sessionKey)).find(entry =>
        entry.provider === current.provider && entry.id === current.model)
      const choices = info?.reasoningEfforts?.length ? info.reasoningEfforts : FALLBACK_REASONING_EFFORTS
      if (!choices.includes(effort as never)) {
...
export function installBridgeModelSelection(agentCtx: unknown, selection: BridgeModelSelectionRef): () => void {
  const scoped = agentCtx as {
    on?: (event: string, cb: (...args: unknown[]) => unknown) => () => void
  }
  if (typeof scoped.on !== 'function') return () => {}

  const disposeAssembly = scoped.on('system-prompt/assemble', async (...args: unknown[]) => {
    const next = args[2]
    if (typeof next !== 'function') return undefined
    const selected = selection.current
    const assembled = await (next as () => Promise<{ variables?: Record<string, unknown> }>)()
  ...
  const disposeRequest = scoped.on('agent/request', async (...args: unknown[]) => {
    const next = args[1]
    if (typeof next !== 'function') return undefined
...
export function resolveConfiguredModels(defaultModel: string, configured: readonly string[] = []): string[] {
...
function formatModels(models: readonly BridgeModelInfo[], current: BridgeModelSelection): string {
  if (models.length === 0) return '当前没有可列出的模型。'
  const lines = [
...
function formatModelIds(models: readonly BridgeModelInfo[]): string {
  if (models.length === 0) return '可选模型列表为空。'
  return `可选模型: ${models.map(model => model.id).join(', ')}`
}

function formatSelection(prefix: string, selected: BridgeModelSelection): string {
  return [
