...
export class NapcatSelfLogInput {
  ...
  constructor(private readonly opts: NapcatSelfLogInputOptions) {}

  async start(onMessage: NapcatSelfLogHandler): Promise<void> {
  ...
  private async poll(onMessage: NapcatSelfLogHandler): Promise<void> {
    let size: number
    try {
...
export function parseNapcatSelfLogLine(
  line: string,
  opts: { selfQq: number; commandPrefix: string },
): { userId: number; rawMessage: string } | null {
  ...
  if (privateMatch) {
    ...
    if (!Number.isFinite(userId) || userId !== opts.selfQq) return null
    if (!rawMessage.startsWith(opts.commandPrefix)) return null
